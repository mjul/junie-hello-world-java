...
public class CustomOAuth2UserService extends DefaultOAuth2UserService implements OAuth2UserService<OAuth2UserRequest, OAuth2User> {
    ...
    public OAuth2User loadUser(OAuth2UserRequest userRequest) {
        ...
        if ("github".equalsIgnoreCase(registrationId)) {
            ...
            profile = new ProviderProfile(registrationId.toUpperCase(),
                    string(attributes.get("sub")),
                    firstNonBlank(string(attributes.get("preferred_username")), string(attributes.get("name"))),
    ...
    private ProviderProfile resolveGithubProfile(Map<String, Object> attributes, OAuth2AccessToken token) {
        ProviderProfile base = ProviderProfileMapper.fromGithub(attributes);
        if (base.email() == null && token != null && token.getScopes() != null && token.getScopes().contains("user:email")) {
            try {
                String email = fetchPrimaryVerifiedGithubEmail(token);
                if (email != null) {
                    return new ProviderProfile(base.provider(), base.externalId(), base.username(), base.displayName(), email, base.avatarUrl());
                }
            } catch (Exception e) {
    ...
    private String fetchPrimaryVerifiedGithubEmail(OAuth2AccessToken token) throws RestClientException {
